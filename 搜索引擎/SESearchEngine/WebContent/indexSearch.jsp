...
<body>
	...
	<div class="navi">
		<ul class="web">
			<li>
				<a href="#" style="color:#6E6E6E;"><h4>网页搜索</h4></a>
			</li>
			<li>
				<a href="/SESearchEngine/timeSearch.jsp?keys=<% out.print(parm); %>" style="color:#6E6E6E;"><h4>时间搜索</h4></a>
			</li>
		</ul>
	...
	<div id="result" class="result">
		<%
			...
			if(results.length == 0) {
				out.print("<div><h3>暂无搜索结果，换换关键字...</h3></div>");
			}
			/* 去重实现  */
			...
			/* for(int i = start; i < end; i++) {
				//显示查询结果
				out.print("<a href=\"http://"+url+"\">"+"<font color=\"#003c00\">"+title+"</a>"+"</font><br/>");
				out.print("<font color=\"#008000\">"+summary+"<a href=\"http://"+url+"\">"+"<font color=\"#003c00\">more</a>"+"</font>...</font><br/>");
				out.print("<font color=\"#cccccc\">"+url+"</font><br/><br/><br/>");
			} */
			...
			for(int i = start; i < end; i++) {
				...
				if(position > 1) {
					...
					String time = results[i].substring(position+5, position+16);
					time = time.trim();
					System.out.println(time);
				}
				...
				if(l == 1) {
					...
					for(int j = 0; j < 1; j++) {
						...
						if("".equals(summary)){
							...
							summary = summary+"..."+"<font color=\"#FF0000\">"+k[0]+"</font>"+context.substring(summaryStart,summaryEnd);
			...
			if(simWords.size() > 0) {
				...
				for(String simWord : simWords) {
					...
					if(results.length > 1) {
						...
						out.print("<div class=\"simkey\"><h5>key:		"+simWord+"</h5></div>");
						out.print("<ul class=\"simTab\">");
						for(String simInfo : re) {
							int position = simInfo.indexOf("|");
							String title = simInfo.substring(0, position);
							position = simInfo.indexOf("mirror");
							String url = simInfo.substring(position+7, simInfo.indexOf("|", position+1));
								
							out.print("<li><a href=\"http://"+url+"\">"+title+"</a></li>");
						}
						out.print("</ul><br></br>"); */
					}
				}
